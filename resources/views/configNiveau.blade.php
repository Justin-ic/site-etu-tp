    ...
    <div class="card-header py-3">
        <div class="row d-flex">
            <div class="col-sm-9 d-flex justify-content-start">
                <h2 class="m-0 font-weight-bold text-primary">Gestion des <b>Niveaux</b></h2>
...
guichet_personnel */ 
 ?></pre> -->
        <tbody>
            <?php $i=1; if ($Liste_nivaux->count() > 0):  if (isset($_GET['page'])) {{ $i=$_GET['page']*5 - 4;}} ?>
                <?php foreach ($Liste_nivaux as $La_Liste_nivaux): ?>
            <tr  class="MCenter">

                <td style="display: none;">{{$La_Liste_nivaux->id}}</td>
                <td><?=$i++ ?></td>
                <td>{{$La_Liste_nivaux->LibelleNiveau}}</td>
                <td>{{$La_Liste_nivaux->filiere->LibelleFiliere}}</td>
                <td>{{$La_Liste_nivaux->updated_at->format('d/m/Y')}}</td>
            </tr> 
                <?php endforeach ?> 
            <?php endif ?>
        </tbody>
  ...
  <div class="modal-dialog">
    ...
    <form action="{{route('niveaux.store')}}" method="POST">
      ...
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="exampleModalLabel">Ajoutez un niveau</h1>
      ...
      <div class="modal-body">
        ...
        <div class="form-group">
            <b>Libelle Niveau:</b>
            <input type="text" name="LibelleNiveau"  class="form-control" required id="" placeholder="L1">
        </div>


        <div class="form-group">
            <b>Filière:</b>
            <select class="form-control" required name="FiliereId" id="modale_etat_anne">
...
var filieres = '{';
<?php foreach ($Liste_Filiere as $key => $Filiere): ?> /*< ?=$Filiere->id ?>  < ?=$Filiere->LibelleFiliere ?>*/
    filieres = filieres+'"<?=$Filiere->id ?>":"<?=$Filiere->LibelleFiliere ?>",';
<?php endforeach ?>

filieres = filieres.slice(0, filieres.length - 1);
filieres = filieres+'}';
// alert(< ?php echo count($Liste_Filiere); ?>);
// alert(filieres.charAt(filieres.length -2));
// alert(filieres);
...
*/
  ...
  $('#editable').Tabledit({
    ...
    columns:{
      ...
      editable:[[2, 'LibelleNiveau'],[3, 'FiliereId',filieres]
